...
export const createRegistration = async (req, res, db) => {
    try {
        ...
        const existingTeamsOnTournament = await db
        ...
        if (existingTeamIds.length > 0) {
            ...
            if (conflictingPlayer) {
                const userDetails = await db
                    .collection('users')
                    .findOne({ _id: conflictingPlayer }, { projection: { username: 1 } })
                const username = userDetails ? userDetails.username : 'A player'
                return res.status(409).json({
                    message: `${username} from your team is already registered for this tournament with another team.`,
                })
            }
        }
        ...
        res.status(201).json({
            message:
                'Team successfully registered for the tournament. Waiting for organizer approval.',
        })
    } catch (error) {
...
export const updateRegistrationStatus = async (req, res, db) => {
    try {
        ...
        const tournament = await db
            .collection('tournaments')
            .findOne({ _id: registration.tournamentId })
        if (!tournament) {
...
export const requestWithdrawal = async (req, res, db) => {
    try {
        ...
        if (!team || !team.captain.equals(requesterId)) {
            return res
                .status(403)
                .json({ message: 'Forbidden: Only the team captain can request a withdrawal.' })
        }
        ...
        await db
            .collection('registrations')
            .updateOne({ _id: registration._id }, { $set: { status: 'pending_withdrawal' } })
