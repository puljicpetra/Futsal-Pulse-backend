...
export const createMatchValidationRules = () => [
  ...
  body('teamB_id')
    .isMongoId().withMessage('Valid Team B ID is required.')
    .custom((value, { req }) => {
...
function buildMatchDetailsPipeline(matchFilter) {
  return [
    ...
    { $lookup: { from: 'users', localField: 'teamA.players', foreignField: '_id', as: 'teamAPlayers' } },
    { $lookup: { from: 'users', localField: 'teamB.players', foreignField: '_id', as: 'teamBPlayers' } },
    {
      $project: {
        _id: 1,
        tournamentId: 1,
...
export const createMatch = async (req, res, db) => {
  ...
  try {
    ...
    const endBase = tournament.endDate ? new Date(tournament.endDate) : new Date(tournament.startDate);
...
export const addMatchEvent = async (req, res, db) => {
  ...
  try {
    ...
    if (!match.teamA_id.equals(teamObjId) && !match.teamB_id.equals(teamObjId)) {
      return res.status(400).json({ message: 'Provided teamId does not belong to this match.' });
    }
    ...
    const finalMatchResult = await db.collection('matches')
