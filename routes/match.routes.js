...
export const createMatchRouter = (db) => {
    ...
    router.get('/', (req, res) => matchController.getAllMatches(req, res, db))
    ...
    router.post('/', authMiddleware, createMatchValidationRules(), (req, res) =>
        matchController.createMatch(req, res, db)
    )

    router.patch('/:matchId/finish', authMiddleware, (req, res) =>
        matchController.finishMatch(req, res, db)
    )

    router.delete('/:matchId', authMiddleware, (req, res) =>
        matchController.deleteMatch(req, res, db)
    )

    router.post('/:matchId/events', authMiddleware, addEventValidationRules(), (req, res) =>
        matchController.addMatchEvent(req, res, db)
    )

    router.delete('/:matchId/events/:eventId', authMiddleware, (req, res) =>
        matchController.deleteMatchEvent(req, res, db)
    )
